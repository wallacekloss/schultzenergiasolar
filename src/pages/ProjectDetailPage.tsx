...
export default function ProjectDetailPage() {
  ...
  return <>
    ...
    <article>
      <header className="pt-32 pb-10 bg-background"><div className="container-max mx-auto px-4 sm:px-6 lg:px-8"><Breadcrumbs items={[{ label: "Projetos", to: "/projetos/" }, { label: project.title }]} /><div className="mt-8 max-w-4xl"><p className="text-primary uppercase font-bold text-sm mb-3">{project.segmentLabel} • {project.location}</p><h1 className="font-display text-4xl md:text-6xl font-bold text-secondary leading-tight mb-5">{project.h1}</h1><p className="text-lg text-muted-foreground">Projeto real registrado no portfólio da Schultz Energia Solar.</p></div></div></header>
      ...
      <section className="section-padding bg-background"><div className="container-max mx-auto grid lg:grid-cols-[1fr_0.6fr] gap-12"><div><h2 className="font-display text-3xl font-bold text-secondary mb-5">Sobre o projeto</h2><p className="text-lg text-muted-foreground leading-relaxed mb-8">{project.description}</p><h2 className="font-display text-3xl font-bold text-secondary mb-5">Solução aplicada</h2><p className="text-lg text-muted-foreground leading-relaxed">{project.solution}</p></div><aside className="bg-muted/50 p-6 rounded-lg h-fit"><h2 className="font-display text-xl font-bold text-secondary mb-5">Informações do projeto</h2><dl className="space-y-5"><div><dt className="text-sm text-muted-foreground">Projeto</dt><dd className="font-bold">{project.title}</dd></div><div><dt className="text-sm text-muted-foreground">Tipo</dt><dd className="font-bold">{project.segmentLabel}</dd></div><div><dt className="text-sm text-muted-foreground flex gap-2"><MapPin className="h-4 w-4" />Localização</dt><dd className="font-bold">{project.location}</dd></div><div><dt className="text-sm text-muted-foreground flex gap-2"><Zap className="h-4 w-4" />Potência instalada</dt><dd className="font-bold text-primary text-xl">{project.power}</dd></div></dl></aside></div></section>
